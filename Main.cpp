...
int main() {
  ...
  // Bot* AOTY = new Bot();
  // std::cout << AOTY->count_material(BLACK);
  // std::vector<Move*> moves = AOTY->getMovePool(BLACK);
  // while (!moves.empty()) {
  //   Move* move = moves.back();
  //   moves.pop_back();
  //   std::cout << move->getSource().value() << " ";
  //   std::cout << move->getDestination().value() << "\n\n";
  // }
  // Move* move = Move::moveTo("e2", "e4");
  // AOTY->recordMove(move, WHITE);

  // move = Move::moveTo("g7", "g5");
  // AOTY->recordMove(move, BLACK);

  // print_BTB(AOTY->general_board[WHITE] | AOTY->general_board[BLACK]);

  // move = Move::moveTo("f1", "c4");
  // AOTY->recordMove(move, WHITE);

  // move = Move::moveTo("e7", "e6");
  // AOTY->recordMove(move, BLACK);

  // print_BTB(AOTY->general_board[WHITE] | AOTY->general_board[BLACK]);

  // move = Move::moveTo("d1", "g4");
  // AOTY->recordMove(move, WHITE);

  // move = Move::moveTo("g8", "h6");
  // AOTY->recordMove(move, BLACK);

  // print_BTB(AOTY->general_board[WHITE] | AOTY->general_board[BLACK]);

  // move = Move::moveTo("g4", "h5");
  // AOTY->recordMove(move, WHITE);

  // print_BTB(AOTY->general_board[WHITE] | AOTY->general_board[BLACK]);

  // std::cout << AOTY->is_legal(h7, h6, BLACK);

  
  // Move* random = AOTY->calculateNextMove();
  // std::cout << random->getSource().value() << " ";
  // std::cout << random->getDestination().value() << "\n";
